...
const badgeColors: Record<string, string> = {
  ...
  ON_LEAVE: "bg-amber-400/15 text-amber-300",
  TERMINATED: "bg-danger/15 text-danger",
  PRESENT: "bg-emerald-400/15 text-emerald-300",
  ABSENT: "bg-danger/15 text-danger",
  HALF_DAY: "bg-amber-400/15 text-amber-300",
  WORK_FROM_HOME: "bg-volt/15 text-volt-soft",
...
export function Badge({ label }: { label: string }) {
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${badgeColors[label] ?? "bg-surface-3 text-text-muted"}`}
    >
      {label.replace("_", " ")}
    </span>
  );
